...
Another example is my neofetch config, which was stored in =~/.config/neofetch/config.conf=. This one I moved to =~/some/nested/dirs/dotfiles/neofetch/.config/neofetch/config.conf=.
...
in =/home/matthias/.emacs.d/init.org=.
